# Running in Azure (Databricks + Synapse)

This repo includes example notebooks under:
- `notebooks/databricks/`
- `notebooks/synapse/`

They assume you store outputs in ADLS Gen2 containers (bronze/silver/gold) and optionally query the Gold data from Synapse.

## Minimal Azure setup (high level)
1. **ADLS Gen2**: create containers `bronze`, `silver`, `gold`.
2. **Databricks**: configure access to ADLS (Unity Catalog external locations or DBFS mount), then run the notebooks in order.
3. **ADF (optional)**: pass `start_date`/`end_date` into the Bronze notebook via widgets and schedule daily.
4. **Synapse (optional)**: create external tables/views over the Gold CSV output.

> Tip: keep your storage account/container names as parameters in the first cell of each notebook so a reviewer can understand how to wire it up quickly.
<br/>
